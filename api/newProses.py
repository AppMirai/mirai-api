from api.preProses import preProses
...
class Proses():
    def __init__(self, warna, tipeMakeUp, query):
        self.imageDetector = preProses(query)
        self.face = Face(tipeMakeUp)
        self.r = warna[0]
        self.g = warna[1]
        self.b = warna[2]

        if(len(self.imageDetector.faces) > 0):
            # start image filter
            for face in self.imageDetector.faces:
                ...
                if tipeMakeUp == 'lips':
                    ...
                    self.face.image = self.face.createBoxLips(self.imageDetector.image, self.face.point, 2, masked=True,
                                                              cropped=False, tipe=tipeMakeUp)
